...
import com.kckc.service.role.RoleService;
import com.kckc.service.user.UserService;
import com.kckc.tools.Constants;
import com.kckc.tools.PageSupport;
...
public class UserController extends BaseController{
    ...
    public String addUserSave(User user,HttpSession session,HttpServletRequest request,
        ...
        if(flag){
            user.setCreatedBy(((User)session.getAttribute(Constants.USER_SESSION)).get_Id());
    ...
    public String modifyUserSave(User user,HttpSession session,HttpServletRequest request,
                                 @RequestParam(value ="attachs", required = false) MultipartFile[] attachs){
        logger.debug("modifyUserSave id===================== "+user.get_Id());
        String idPicPath = null;
        String workPicPath = null;
        String errorInfo = null;
        boolean flag = true;
        ...
        if(flag){
            user.setModifyBy(((User)session.getAttribute(Constants.USER_SESSION)).get_Id());
    ...
    public String pwdSave(@RequestParam(value="newpassword") String newPassword,
        ...
        if(o != null && !StringUtils.isNullOrEmpty(newPassword)){
            try {
                flag = userService.updatePwd(((User)o).get_Id(),newPassword);
            } catch (Exception e) {
                e.printStackTrace();
            }
            if(flag){
